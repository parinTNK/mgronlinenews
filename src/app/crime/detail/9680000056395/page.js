...
export default function Home() {
  ...
  // Get current pathname to use in share URLs
  const pathname = usePathname();
  const currentUrl = `https://mgronline.com${pathname}`;
  const encodedUrl = encodeURIComponent(currentUrl);
  const encodedTitle = encodeURIComponent(
    "ตำรวจไซเบอร์เร่งขยายผล ปิดบัญชีธนาคารและเว็บไซต์พนันออนไลน์เครือข่ายใหญ่รวม 78 แห่ง โยงบริษัท Huione ในกัมพูชา เอี่ยวหลาน \"ฮุนเซน\""
  );

  // Facebook share URL
  const facebookShareUrl = `https://www.facebook.com/share_channel/?type=reshare&link=${encodedUrl}&app_id=574706704498021&source_surface=external_reshare&display&hashtag`;

  // Twitter/X share URL
  const twitterShareUrl = `https://x.com/intent/tweet?original_referer=${encodedUrl}&text=${encodedTitle}&url=${encodedUrl}&original_referer=${currentUrl}`;
            ...
            <div className="mt-3">
              ...
              <div className="mt-4 flex items-center justify-between">
                <div className="flex space-x-2">
                  ...
                  <a href={lineShareUrl} className="rounded-full bg-[#00b900] p-[2px]">
                    <Image
